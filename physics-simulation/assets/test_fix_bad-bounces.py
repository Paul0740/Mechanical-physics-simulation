...
@wp.kernel
def resolve_collision_serial(mass: wp.array(dtype=wp.float32),
                             pos: wp.array(dtype=wp.vec2f),
                             vel: wp.array(dtype=wp.vec2f),
                             radius: float):
    tid = wp.tid()
    n = mass.shape[0]
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if dist <= 2.0 * radius:
                unit_v = diff / dist
                rel_vel = vel[i] - vel[j]
                factor = 2.0 * wp.dot(rel_vel, unit_v) / (mass[i] + mass[j])
                vel[i] -= factor * mass[j] * unit_v
                vel[j] += factor * mass[i] * unit_v

                # Correct positions safely
                overlap = 2.0 * radius - dist
                if overlap > 0.0:
                    correction = 0.5 * overlap
                    pos[i] += unit_v * correction
                    pos[j] -= unit_v * correction
...
for step in range(num_frames):
    ...
    wp.launch(resolve_collision_serial, dim=(n,n), inputs=[mass_gpu, pos_gpu, vel_gpu, radius])
